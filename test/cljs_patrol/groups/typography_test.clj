...
(deftest analyze-test
  ...
  (testing "single token group — no violation"
    (let [result (group/analyze typography/group
  ...
  (testing "violations are per declaration"
    (let [kw-b :webapp.styles/other-style
          result (group/analyze typography/group
                                {:usages [(token-ref kw "action-small-font")
                                          (token-ref kw "action-large-letter-spacing")
...
(deftest failed?-test
  (testing "fails when mixed token groups exist"
    (is (group/failed? typography/group {:mixed-token-groups [{:decl-kw kw}]})))

  (testing "does not fail when no violations"
    (is (not (group/failed? typography/group {:mixed-token-groups []})))))

(deftest summary-lines-test
  (let [lines (group/summary-lines typography/group {:mixed-token-groups [{:decl-kw kw} {:decl-kw kw}]})]
    (is (= 1 (count lines)))
