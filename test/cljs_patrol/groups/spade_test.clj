...
(deftest analyze-test
  (testing "no styles declared — nothing unused"
    (is (empty? (:unused-styles (spade/analyze {:declarations [] :usages []})))))

  (testing "declared and used — not unused"
    (let [result (spade/analyze {:declarations [defclass-decl] :usages [style-usage]})]
      (is (empty? (:unused-styles result)))))

  (testing "declared but not used — unused"
    (let [result (spade/analyze {:declarations [defclass-decl defattrs-decl] :usages []})]
  ...
  (testing "ignores non-spade declaration types"
    (let [other {:kw :webapp/sub :type :sub :file "f.cljs" :row 1}
          result (spade/analyze {:declarations [defclass-decl other] :usages []})]
      (is (= 1 (count (:unused-styles result)))))))

(deftest failed?-test
  (testing "fails when unused styles exist"
    (is (spade/failed? {:unused-styles [defclass-decl]})))

  (testing "does not fail when no unused styles"
    (is (not (spade/failed? {:unused-styles []})))))

(deftest summary-lines-test
  (let [lines (spade/summary-lines {:unused-styles [defclass-decl defattrs-decl]})]
    (is (= 1 (count lines)))
